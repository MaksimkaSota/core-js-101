...
function doRectanglesOverlap(rect1, rect2) {
  const firstLine = rect1.left + rect1.width > rect2.left;
  const secondLine = rect1.top + rect1.height > rect2.top;
...
function isInsideCircle(circle, point) {
  ...
  return pointX + pointY < circle.radius ** 2;
}
...
function getDigitalRoot(num) {
  const answer = String(num).split('').reduce((sum, current) => sum + Number(current), 0);
...
function getCommonDirectoryPath(pathes) {
  ...
  arr[0].forEach((itemOne, index) => {
    if (arr.every((itemTwo) => itemOne === itemTwo[index])) {
      answer += `${itemOne}/`;
    }
  });
